...
| `QTLBurdenOutlierEnrichment.tsv` | Observed expression and proteomics outlier enrichment by burden bin among protein-coding genes. Models include `AllCalls`, `HighConfidence`, fixed high-KO removal thresholds such as `HighKORemoved_NumKOlt50`, legacy `HighKORemoved` (`NumKO < 100`), `HighKORemoved_NumKOlt250`, and top-`NumKO` percentage filters such as `HighKORemoved_Top0.5Pct` and `HighKORemoved_Top1Pct`. Rows include protein-coding KO-filter metadata columns. Empty (header-only) when no outlier annotations are provided. |
